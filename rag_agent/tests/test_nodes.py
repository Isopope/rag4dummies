...
def test_agent_action_search_documents(config, mock_query_tool):
    ...
    # La signature de requête inclut le source_name (vide ici) : "::budget 2024"
    assert any("budget 2024" in sig for sig, _ in result["seen_queries"])
...
def test_agent_action_duplicate_query_skipped(config, mock_query_tool):
    ...
    import json
    state = create_unified_state("Q ?")
    # La signature inclut le source_name vide : "::budget 2024"
