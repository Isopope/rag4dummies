...
def test_generate_post(config):
    from rag_agent.nodes.generation import generate_post
    ...
    def mock_llm_call(messages, **kwargs):
        ...
        if "assistant qui génère des questions de suivi" in sys_msg:
            return _make_llm_call('["Question 1 ?", "Question 2 ?"]')(messages, **kwargs)
        else:
            return _make_llm_call("Politique budgétaire 2024")(messages, **kwargs)
            
    result = generate_post(state, llm_call=mock_llm_call, rag_config=config)
    assert len(result["follow_up_suggestions"]) <= 3
    assert result["conversation_title"] == "Politique budgétaire 2024"
