          ...
          {['Résume le document principal', 'Quels sont les points clés ?', 'Compare les sources disponibles'].map(
            (s) => (
              <button
                key={s}
                onClick={() => onSelect(s)}
                className="px-5 py-2 text-xs rounded-full border border-border/70 bg-card/80 backdrop-blur-sm hover:bg-accent hover:text-accent-foreground hover:border-accent text-foreground transition-all duration-200 font-medium shadow-sm"
              >
                {s}
              </button>
            ),
